...
Metric & GCN+MLP \\
\midrule
ECE (deterministic)           & 0.0206 \\
MC Dropout mean uncertainty   & 0.0530 \\
\bottomrule
\end{tabular}
\end{center}
...
Higher uncertainty on OOD pairs provides a safety signal for clinical use.
...
\midrule
MLP      & 0.9660 & 0.5959 & $-0.370$ \\
GCN+MLP  & 0.9675 & 0.8711 & $-0.096$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=*,noitemsep,topsep=1pt]
  ...
  \item \textbf{GCN+MLP is well-calibrated} (low ECE) and the MC Dropout
        uncertainty is elevated for OOD drug pairs, making it suitable as a
        safety signal for clinical decision support.
\end{enumerate}
